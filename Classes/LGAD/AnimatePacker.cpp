...
void AnimatePacker::loadAnimations(const char *path )
{
	...
	CCLog("plists.size() %d",plists.size());
	for (unsigned int i=0;i<plists.size();i++)
	{
/*		CCLog("plists[i].c_str() %s",plists[i].c_str());*/
	    string plistPath = CCFileUtils::sharedFileUtils()->fullPathFromRelativeFile(plists[i].c_str(), pszPath);
// 		CCLog("plistPath.c_str() %s ",plistPath.c_str());
// 		CCLog("pszPath %s",pszPath);
	...
	for (unsigned int i=0;i<animates.size();i++)
	{
		...
		/*	CCLog("spriteFrames[j].c_str() %s",spriteFrames[j].c_str());*/
			...
			animationFrame->initWithSpriteFrame(spriteFrame,animate.delay,NULL);
			spriteFramesArray->addObject(animationFrame);
		}
		CCAnimation *animation=CCAnimation::create(spriteFramesArray,animate.delay);
